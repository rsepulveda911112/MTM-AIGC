...
class ClassificationModelMult:
    def __init__(
        ...
        MODEL_CLASSES = {
            ...
            "rembert": (
                RemBertConfig,
                RemBertForSequenceClassificationMult,
                RemBertTokenizerFast,
            ),
            "roberta": (
                RobertaConfig,
        ...
        if self.args.labels_list and not self.args.lazy_loading:
            if labels_list:
                assert labels_list == len(self.args.labels_list)
        ...
        if num_labels:
            self.config = config_class.from_pretrained(
                model_name, labels_list=num_labels, **self.args.config
            )
        ...
        if self.args.onnx:
            ...
            if not self.args.quantized_model:
                if args['value_head'] == 0:
                    kwargs['labels_list'] = labels_list
                    kwargs['value_head'] = args['value_head']
    ...
    def train_model(
        ...
        if self.args.use_hf_datasets:
            ...
            for i, label_name in enumerate(labels_name):
                ...
                if "text" in train_df.columns and label_name in train_df.columns:
                    if self.args.model_type in ["layoutlm", "layoutlmv2"]:
                        train_examples = [
                ...
                train_dataset_aux = self.load_and_cache_examples(train_examples, verbose=verbose)
                if i == 0:
                    train_dataset = train_dataset_aux
                train_dataset.examples.data[f"labels_{i}"] = train_dataset_aux.labels
                # setattr(train_dataset.examples, f"labels_{i}", train_dataset_aux.labels)
        ...
        # Para probar luego con los mismos textos pero diferentes labels por tareas
        # multi_train_dataset = train_dataset[0]
        # for i in range(1, len(train_dataset)):
        #     # vv = train_dataset[i]
        #     # value = (labels_name[i])
        #     # multi_train_dataset.__setelement__(multi_train_dataset, i, value)
        #     setattr(multi_train_dataset, f"{labels_name[i]}", train_dataset[i].labels)
    ...
    def train(
        ...
        for _ in train_iterator:
            ...
            # batch_iterator = tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc=f"Running Epoch {epoch_number} of {args.num_train_epochs}", disable=args.silent, mininterval=0)
            batch_iterator = tqdm(
                train_dataloader,
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if steps_trained_in_current_epoch > 0:
                    ...
                    continue
                # for task_id, batch in enumerate(batch_per_dls):
                # for batch in batch_zip:

                tasks_id = list(range(0, len(labels_name)))
                inputs_aux = {}
                for i in tasks_id:
                    if f'labels_{i}' in batch[0].keys():
                        if 'list_label' not in inputs_aux.keys():
                            inputs_aux['list_label'] = []
                        inputs_aux['list_label'].append(batch[0][f'labels_{i}'].to(self.device))
                        batch[0].pop(f'labels_{i}')
                    else:
                        break

                inputs = self._get_inputs_dict(batch)
                inputs.pop('labels')
                inputs['list_label'] = inputs_aux['list_label']
                ...
                if self.args.fp16:
                    with amp.autocast():
                        loss, *_ = self._calculate_loss(
                            ...
                            args=self.args, tasks_id=tasks_id
                        )
                else:
                    loss, *_ = self._calculate_loss(
                        model,
                ...
                #Revisar si sería aquí donde se debe poner la union de los loss
            ...
            for task_id, label_name in enumerate(labels_name):
                if args.evaluate_during_training and args.evaluate_each_epoch:
                    ...
                    training_progress_scores[f"global_step_{task_id}"].append(global_step)
                    training_progress_scores[f"train_loss_{task_id}"].append(current_loss)
                    for key in results:
                        training_progress_scores[key].append(results[key])
                    if test_df is not None:
                        test_results, _, _ = self.eval_model(
    ...
    def evaluate(
        ...
        if self.args.use_hf_datasets:
            ...
            if "text" in eval_df.columns and label_name in eval_df.columns:
                if self.args.model_type in ["layoutlm", "layoutlmv2"]:
                    eval_examples = [
                        InputExample(i, text, None, label, x0, y0, x1, y1)
                        for i, (text, label, x0, y0, x1, y1) in enumerate(
        ...
        for i, batch in enumerate(
            ...
            with torch.no_grad():
                inputs = self._get_inputs_dict(batch)
                inputs['list_label'] = [inputs['labels']]
                inputs.pop('labels')
                ...
                if self.args.fp16:
                    with amp.autocast():
                        outputs = self._calculate_loss(
                            ...
                            loss_fct=self.loss_fct,
                            num_labels=self.num_labels,
                            args=self.args, tasks_id=[task_id]
                        )
            ...
            out_label_ids[start_index:end_index] = (
                inputs["list_label"][0].detach().cpu().numpy()
            )
        ...
        result, wrong = self.compute_metrics(
            preds, model_outputs, out_label_ids, eval_examples, task_id=task_id, **kwargs
        )
        result[f"eval_loss_{task_id}"] = eval_loss
    ...
    def compute_metrics(
        ...
        for metric, func in kwargs.items():
            if metric.startswith("prob_"):
                extra_metrics[metric+f"_{task_id}"] = func(labels, model_outputs)
            elif metric == 'f1':
                extra_metrics[metric+f"_{task_id}"] = func(labels, preds, average='macro')
            else:
                extra_metrics[metric+f"_{task_id}"] = func(labels, preds)
        ...
        if num_labels == 2:
            ...
            if self.args.sliding_window:
                return (
                    {
                        **{f"mcc_{task_id}": mcc, f"tp_{task_id}": tp, f"tn_{task_id}": tn, f"fp_{task_id}": fp, f"fn_{task_id}": fn},
                        **extra_metrics,
                    },
                    wrong,
                )
                ...
                return (
                    {
                        **{
                            ...
                            f"fp_{task_id}": fp,
                            f"fn_{task_id}": fn,
                            f"auroc_{task_id}": auroc,
                            f"auprc_{task_id}": auprc,
                        },
                        **extra_metrics,
                    },
                    wrong,
            ...
            return {**{f"mcc_{task_id}": mcc}, **extra_metrics}, wrong

    def predict(self, to_predict, multi_label=False, task_id=None):
        """
        ...
        if not multi_label and self.args.onnx:
            ...
            if self.config.output_hidden_states:
                ...
                for i, batch in enumerate(
                    ...
                    if preds is None:
                        preds = logits.detach().cpu().numpy()
                        out_label_ids = inputs["list_label"][0].detach().cpu().numpy()
    ...
    def _calculate_loss(self, model, inputs, loss_fct, num_labels, args, tasks_id):
        outputs = model(**inputs, tasks_id=tasks_id)
